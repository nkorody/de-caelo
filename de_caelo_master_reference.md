...
2. Ephemeris service: wrap the three existing Python scripts behind two endpoints. `POST /compute-chart` (birth data in, `{natal, progressions}` out) runs at signup, once per user. `POST /refresh-transits` (no user-specific input) recomputes the shared calendar-only transit window and writes it to `sky_snapshots` directly using a service-role key, run by maintenance/cron rather than triggered by end users, ideally not reachable over the public internet at all (see §4.4). Deploy somewhere that tolerates a cold start, neither endpoint is a hot path.
...
- **Ephemeris service**: only runs on signups and occasional maintenance, so a tier that sleeps when idle is fine here (unlike Supabase). Render's free tier (cold start after 15 min idle) is reasonable; $7/month buys always-warm if the cold start bothers onboarding.
